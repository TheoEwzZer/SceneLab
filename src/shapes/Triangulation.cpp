...
float Triangulation::det(const glm::vec2& a, const glm::vec2& b, const glm::vec2& c)
{
    ...
    const std::vector<glm::vec3> &pts3D)
{
    ...
    std::vector<glm::vec2> pts2D(n);
        ...
        const uint32_t i1 = ptsExt.size(); ptsExt.push_back(p1);
        const uint32_t i2 = ptsExt.size(); ptsExt.push_back(p2);
        const uint32_t i3 = ptsExt.size(); ptsExt.push_back(p3);
...
void Triangulation::registerRenderable(const int id)
{
    m_renderableId = id;
}

void Triangulation::updateGeometry(ARenderer &renderer) const
{
    if (m_renderableId == -1) return;
    ...
    for (const auto triangles = triangulate(controls); const auto &
        [a, b, c] : triangles) {
...
bool Triangulation::isRenderable() const
{
    return m_renderableId != -1;
}

bool Triangulation::hasPoints() const
{
