...
void CameraManager::setFocused(int id)
{
    m_FocusedCamID = id;
}
...
void CameraManager::setRotation(const float pitch, const float yaw, const float roll)
{
    if (auto* cam = getFocusedCamera())
        cam->setRotation(pitch, yaw, roll);
}

void CameraManager::setProjection(
...
const glm::vec3 &CameraManager::getPosition() const
{
    if (auto* cam = getFocusedCamera())
...
const glm::vec3 &CameraManager::getRotation() const
{
    if (auto* cam = getFocusedCamera())
