...
void ARenderer::createCameraViews(const int id, int width, int height)
{
    if (!m_cameraViews.contains(id)) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, view.colorTex, 0);
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, view.depthRBO);
...
void ARenderer::renderDockableViews(CameraManager &cameraManager)
{
    for (auto& [id, view]: m_cameraViews) {
        ...
        if (view.size.x != newW || view.size.y != newH) {
            ...
            glBindTexture(GL_TEXTURE_2D, view.colorTex);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, avail.x, avail.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

            glBindRenderbuffer(GL_RENDERBUFFER, view.depthRBO);
            glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, avail.x, avail.y);

            glBindFramebuffer(GL_FRAMEBUFFER, 0);

            if (auto* cam = cameraManager.getCamera(id)) {
                const float aspect = static_cast<float>(view.size.x) / static_cast<float>(view.size.y);
                cam->setProjection(45.0f, aspect, 0.01f, 100.0f);
            }
        }
        ImGui::Image((void*)(intptr_t)view.colorTex, avail, ImVec2(0,1), ImVec2(1, 0));
