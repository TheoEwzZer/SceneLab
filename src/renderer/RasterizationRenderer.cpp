...
void RasterizationRenderer::beginFrame()
{
    ...
    void RasterizationRenderer::drawAll()
    {
        ...
        // First, find and update light position from the light object
        for (const auto& obj : m_renderObjects) {
            if (obj.isLight && obj.isActive) {
                // Extract light position from model matrix (translation component)
                glm::vec3 lightPos = glm::vec3(obj.modelMatrix[3]);

                // Update light position in the lighting shader
                m_lightingShader.use();
                m_lightingShader.setVec3("lightPos", lightPos);
                break;  // Assuming only one light for now
            }
        }
        ...
        for (size_t i = 0; i < m_renderObjects.size(); ++i) {
            ...
            if (err != GL_NO_ERROR) {
                std::cout << "OpenGL error during draw call for object " << i
                         << ": " << err << std::endl;
            }
        }
    }
