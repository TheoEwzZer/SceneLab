...
class ParametricCurve {
    ...
    void updateGeometry(ARenderer& renderer);

    private:
    static float tj(const glm::vec3 &p0, const glm::vec3 &p1, float alpha = 0.5f);
    glm::vec3 catmullRomSample(const glm::vec3 &P0, const glm::vec3 &P1,
                                  const glm::vec3 &P2, const glm::vec3 &P3,
                                  float t);
    std::vector<float> buildCatmullRomPoints(const std::vector<glm::vec3> &controls,
