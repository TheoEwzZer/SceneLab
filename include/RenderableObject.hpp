...
struct Vertex {
    ...
    static void computeTangents(std::vector<Vertex> &vertices)
    {
        for (size_t i = 0; i + 2 < vertices.size(); i += 3) {
            ...
            glm::vec3 bitangent = r * (-deltaUV2.x * edge1 + deltaUV1.x * edge2);
        ...
        for (auto &v : vertices) {
            ...
            float handedness = (glm::dot(glm::cross(N, T), v.bitangent) < 0.0f) ? -1.0f : 1.0f;
            v.tangent = T;
            v.bitangent = glm::cross(N, T) * handedness;
        }
    }

    static std::vector<Vertex> convertToVertex14(const std::vector<float>& vertices)
    {
