...
public:
    TransformManager(SceneGraph& sceneGraph, std::unique_ptr<ARenderer>& renderer);
    ...
    const std::vector<SceneGraph::Node*>& getSelectedNodes() const { return m_selectedNodes; }
    bool canAddToSelection(SceneGraph::Node* nodeToAdd);

    // Transform UI
    void renderTransformUI(bool leftShiftPressed);
    void renderCameraGizmo(int cameraId, const Camera& camera, 
                          ImVec2 imagePos, ImVec2 imageSize, bool isHovered);

    // Gizmo operations
    GizmoOp getCurrentGizmoOperation() const { return m_currentGizmoOperation; }
    void setGizmoOperation(GizmoOp op) { m_currentGizmoOperation = op; }

    // Bounding boxes
    void toggleBoundingBoxes() { m_showAllBoundingBoxes = !m_showAllBoundingBoxes; }
